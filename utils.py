"""
 modify https://github.com/carpedm20/DCGAN-tensorflow/blob/master/utils.py
 
 This file is a utility class.
...
def visualize(sess, dcgan, config, option):
    ...
    if option == 0:
        ...
        save_images(
            samples, [
                image_frame_dim, image_frame_dim], './samples/test_%s.png' %
            strftime(
                "%Y-%m-%d-%H-%M-%S", gmtime()))
    elif option == 1:
        ...
        for idx in range(dcgan.z_dim):
            ...
            save_images(
                samples, [
                    image_frame_dim, image_frame_dim], './samples/test_arange_%s.png' %
                (idx))
    elif option == 2:
        values = np.arange(0, 1, 1. / config.batch_size)
        for idx in [random.randint(0, dcgan.z_dim - 1)
                    for _ in range(dcgan.z_dim)]:
            print(" [*] %d" % idx)
